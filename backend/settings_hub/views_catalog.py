...
class ImageUploadView(APIView):
    ...
    def post(self, request):
        ...
        # 1. Optimize, sanitize metadata, downscale and convert to WebP <= 500KB
        try:
            ...
            return Response(
                {"success": False, "message": f"Failed to process image: {str(e)}", "error": f"Failed to process image: {str(e)}"},
        ...
        if not success:
            ...
            return Response(
                {"success": False, "message": error_msg or "Failed to upload image to storage.", "error": error_msg or "Failed to upload image to storage."},
                status=status.HTTP_502_BAD_GATEWAY,
            )

        # 4. If replacing an existing image directly
        ...
        if old_image_path:
            try:
                ...
                logger.warning(f"Failed to delete old asset '{old_image_path}': {del_err}")
