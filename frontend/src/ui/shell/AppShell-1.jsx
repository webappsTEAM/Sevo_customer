...
export function AppShell() {
  ...
  useEffect(() => {
    // A parent's drill-down panel should stay open for its OWN route and for
    // any of its children's routes — children often live at their own
    // top-level path (e.g. /returns, /claims) rather than nested under the
    // parent's path (e.g. /inventory), so matching on item.to alone caused
    // the whole panel to disappear as soon as you navigated into a child.
    function pathMatches(to) {
      if (!to) return false
    ...
    const parent = items.find(item => {
      ...
      if (pathMatches(item.to)) return true
      return item.children.some(child => pathMatches(child.to))
    })

    setDrillDownParent(parent || null)
