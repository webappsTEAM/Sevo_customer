...
export function CustomerTrackingMap({
  ...
  const [mapLayer, setMapLayer] = useState("google_streets") // 'google_streets' | 'satellite' | 'dark'
  ...
  useEffect(() => {
    if (!rawTechnicianPos || !destinationPos || isTerminal || isArrived || isInProgress) {
      setRoadRoute([])
    ...
    if (!shouldFetch) {
      ...
      if (movedM >= 60 && elapsedSec >= 20) shouldFetch = true
    }
  ...
  }, [rawTechnicianPos?.[0], rawTechnicianPos?.[1], destinationPos?.[0], destinationPos?.[1], isTerminal, isArrived, isInProgress])
  ...
  useEffect(() => {
    ...
    // Road snapping safeguard: project point on road route if within 25m
    const targetGps = hasRoadGeometry && roadRoute.length > 1
      ? projectPointOnPolyline(rawTechnicianPos, roadRoute, 25)
      : rawTechnicianPos
    ...
    // Smooth animation duration (1.0s to 1.2s depending on distance)
    const animDuration = Math.min(1400, Math.max(800, distM * 30))
    ...
    const stepAnimation = (now) => {
      ...
      if (bikeMarkerRef.current) {
        ...
        // Periodically update marker icon bearing
        bikeMarkerRef.current.setIcon(createServiceVehicleMarker({
          category: vehicleCategory,
          bearing: currBearing,
          status,
  ...
  return (
    <div className="ltp-map-container-wrap">
      {/* ── Floating Control Bar (Top Right): Tile Switcher & Recenter Button ── */}
      <div className="ltp-map-top-bar">
        ...
        <div className="ltp-layer-switcher">
          {[
            ...
            <button
              key={l.id}
              className={`ltp-layer-btn ${mapLayer === l.id ? "active" : ""}`}
      ...
      >
        ...
        {/* ── 1. Road Route Polyline ── */}
        {hasRoadGeometry && roadRoute.length > 1 && !isArrived && !isInProgress && !isTerminal && (
          <>
            {/* Soft Outer Polyline Glow */}
            <Polyline
              positions={roadRoute}
              pathOptions={{
                color: "#38bdf8",
        ...
        {/* Straight-line Polyline Fallback if OSRM unavailable */}
        {!hasRoadGeometry && rawTechnicianPos && destinationPos && !isArrived && !isInProgress && !isTerminal && (
          <Polyline
        ...
        {/* ── 2. Customer Destination Marker & Geofence (5m Radius) ── */}
        {destinationPos && (
          <>
            <Circle
        ...
        {/* ── 3. Live Technician Vehicle Marker ── */}
