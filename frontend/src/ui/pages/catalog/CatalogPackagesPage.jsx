...
export function CatalogPackagesPage() {
  ...
  return (
    <div style={{ animation: "fadeUp 0.3s ease both" }} className="p-4 sm:p-6 lg:p-8 w-full max-w-[1720px] mx-auto font-sans text-slate-800 space-y-5">
      ...
      {activeCategoryServicesWithPackages.length > 0 && (
        <div className="bg-white rounded-2xl p-3.5 shadow-xs border border-slate-200/90 mb-5">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-thin">
            ...
            {/* Dedicated AC Inspection tab when viewing AC & Appliances */}
            {activeCategoryKey === "ac_appliance" && (
              <button
                type="button"
                onClick={() => setActiveSubServiceKey("ac-inspection")}
                className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer border shrink-0 ${
                  activeSubServiceKey === "ac-inspection"
                    ? "bg-amber-500 text-slate-950 border-amber-500 shadow-sm"
                    : "bg-amber-50/70 text-amber-900 border-amber-200 hover:bg-amber-100"
                }`}
              >
                <Wrench className="w-3.5 h-3.5 text-amber-700" />
                <span>AC Inspection &amp; Rates</span>
                <span className="text-[10px] font-black px-1.5 py-0.5 rounded-md bg-amber-200 text-amber-900">
                  ₹{acInspectionConfig?.fee || 199}
                </span>
              </button>
            )}
          </div>
      ...
      {loading ? (
      ...
      ) : (visibleServices.length === 0 || visibleServices.every((s) => s.packages.length === 0 && s.totalPackages === 0)) && activeSubServiceKey !== "ac-inspection" ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200/80 shadow-sm">
        ...
        <div className="space-y-4">
          {/* AC Inspection & Rate Card Management Card for Super Admin */}
          {activeCategoryKey === "ac_appliance" && (activeSubServiceKey === "all" || activeSubServiceKey === "ac-inspection") && (
            <div className="bg-gradient-to-r from-amber-50 via-white to-amber-50/50 rounded-2xl border-2 border-amber-300/80 p-5 shadow-xs">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="text-base font-black text-slate-900">
                        AC Inspection &amp; Diagnostic Visit Rate Card
                      </h3>
                      <span className="px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-900 border border-amber-300 text-[10px] font-black uppercase tracking-wider">
                        Doorstep Diagnostic &amp; Spares
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 font-medium mt-1 max-w-xl leading-relaxed">
                      Customise the diagnostic fee, doorstep checklist, and spare parts rate card directly in PostgreSQL database.
                    </p>
                    <div className="flex items-center gap-3 mt-2 text-xs font-bold text-slate-600 flex-wrap">
                      <span className="text-emerald-700 font-extrabold">
                        Diagnostic Visit Fee: ₹{acInspectionConfig?.fee ?? 199}
                      </span>
                      <span>•</span>
                      <span>{acInspectionConfig?.rateCardCategories?.length || 0} Rate Categories</span>
                      <span>•</span>
                      <span>
                        {(acInspectionConfig?.rateCardCategories || []).reduce((s, c) => s + (c.items?.length || 0), 0)} Parts &amp; Services Configured
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
                  <button
                    ...
                    className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-amber-300 hover:text-amber-200 text-xs font-black transition-all cursor-pointer shadow-sm border border-slate-700 active:scale-98"
                  >
                    <SlidersHorizontal className="w-3.5 h-3.5 text-amber-400" />
                    <span>Customise AC Inspection</span>
                  </button>
                  <a
                    href={routes.catalog_ac_inspection_rates}
                    className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-amber-100 hover:bg-amber-200 text-amber-900 text-xs font-black transition-all cursor-pointer border border-amber-300/70"
                  >
          ...
          {activeSubServiceKey !== "ac-inspection" && visibleServices.map((svcItem, idx) => {
      ...
      {/* ── AC Inspection Customizer Modal for Super Admin ── */}
      {showAcInspectionCustomizer && (
        <ACInspectionCustomizerModal
          ...
          onSaved={(newCfg) => {
            ...
            showToast("AC Inspection & spare parts rates saved successfully", "success")
          }}
        />
      )}
    </div>
