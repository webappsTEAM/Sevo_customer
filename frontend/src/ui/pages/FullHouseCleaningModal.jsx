...
export function FullHouseCleaningModal({ activeSubTab: propActiveSubTab, cart, setCart, onClose, onCheckout }) {
  ...
  const getModalPrice = () => {
    ...
    const kitchenPrice = selectedKitchenOpt?.price || 0;
    const sofaPrice = selectedSofaOpt?.price || 0;
    const cupboardPrice = selectedCupboardOpt?.price || 0;
    const floorPrice = selectedFloorOpt?.price || 0;
    const extrasPrice = selectedExtraOpts.reduce((sum, opt) => sum + (opt.price || 0), 0);
    const partialPrice = selectedPartialOpts.reduce((sum, opt) => sum + (opt.price || 0), 0);
